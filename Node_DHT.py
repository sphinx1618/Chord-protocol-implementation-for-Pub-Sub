...
class DataStore:
    ...
    def insert(self, key, value):
        ...
        self.subscribers[int(key)] = {"Port":value,"Text":[]}

        pblist = self.fetch_pub()
        
        sock = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
        port = int(value)  # ports of the servers
        sock.connect(('127.0.0.1',port))
        msg = json.dumps(pblist)
        sock.send(msg.encode('utf-8'))
        print("Subscriber added",self.subscribers)

    def insert_pub(self,Name):
    ...
    def adding_subscriptions(self,names,key):
        initial = self.server
        names = set(names)
        for i in names:
            if self.publishers.get(i,-1)!=-1:
                self.publishers[i].append(key)
        print(self.publishers)

    def adding_subscriptions_helper(self,names,key):
        self.adding_subscriptions(names,key)
        db = open('pk','rb')
        st = pickle.load(db)
        db.close()
        for curr_port in st[0]:
            if(self.server.port != curr_port):
                text = 'subscriptions|' + str(key) + ":" + " ".join(names)
                sock = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
                sock.connect(('127.0.0.1',curr_port))
                sock.send(text.encode('utf-8'))

    def send_articles(self,Name,text):

        for subs in self.publishers[Name]:

            if(subs in self.subscribers):
                port_val = self.subscribers[subs]['Port']

                sock = socket.socket(socket.AF_INET,socket.SOCK_STREAM)
                port = int(port_val)  # ports of the servers
                sock.connect(('127.0.0.1',port))
                
                sock.send(text.encode('utf-8'))
            else:
                name = " ".join(str(item) for item in self.publishers[Name])
                id_of_key = self.server.hash(str(subs))
                succ = self.server.find_successor(id_of_key)
                ip,port = self.server.get_ip_port(succ)
                self.server.request_handler.send_message(ip,port,"send_articles_helper|" + name + ":" + text)


        return "Marjaaa"

    def send_articles_helper(self,subscriber,text):
    ...
    def search(self, search_key):
        ...
        if search_key in self.subscribers:
            return self.subscribers[search_key]
        else:
            # print('Not found')
...
class Node:
    ...
    def process_requests(self, message):
        ...
        if operation == "delete_server":
            ...
            self.data_store.subscribers.pop(data)
            result = 'Deleted'

        if operation == "search_server":
            # print('searching in my datastore', str(self.nodeinfo))
            data = message.split('|')[1]
            if data in self.data_store.subscribers:
                return self.data_store.subscribers[data]
            else:
                return "NOT FOUND"
        ...
        if operation == "insert_pub":
            # print("finding hop to insert the key" , str(self.nodeinfo) )
            data = message.split('|')
            Name = data[1]
            result = self.data_store.insert_pub(Name)

        if operation == "subscriptions":
            data = message.split('|')[1]
            key,names = data.split(':')
            names = names.split()
            result = self.data_store.adding_subscriptions(names,int(key))

        if operation == "subscriptions_helper":
            data = message.split('|')[1]
            key,names = data.split(':')
            names = names.split()
            result = self.data_store.adding_subscriptions_helper(names,int(key))

        if operation == "articles":
            data = message.split('|')[1]
            Name,text = data.split(':')
            
            result = self.data_store.send_articles(Name,text)

        if operation == "send_articles_helper":
            data = message.split('|')[1]
            Name,text = data.split(':')
            Name = Name.split(" ")
            result = self.data_store.send_articles_helper(Name,text)
    ...
    def insert_key(self,key,value):
        ...
        # self.data_store.subscribers[str(key)] = {'Port':str(value), 'Text':[]}
    ...
    def search_key_helper(self,key):
        '''
        The function to handle the incoming key_value pair search request from the client this function searches for the
        correct node on which the key_value pair is stored and then sends a message to that node to return the value 
        corresponding to that key.
        ...
        if self.successor.id != self.id:
            ...
            for key_value in data.split(':'):
                if len(key_value) > 1:
                    ...
                    self.data_store.subscribers[key_value.split('|')[0]] = key_value.split('|')[1]
    ...
    def send_keys(self, id_of_joining_node):
        ...
        data = ""
        keys_to_be_removed = []
        for keys in self.data_store.subscribers:
            ...
            if self.get_forward_distance_2nodes(key_id , id_of_joining_node) < self.get_forward_distance_2nodes(key_id,self.id):
                data += str(keys) + "|" + str(self.data_store.subscribers[keys]) + ":"
                keys_to_be_removed.append(keys)
        for keys in keys_to_be_removed:
            self.data_store.subscribers.pop(keys)
        return data
    ...
    def stabilize(self):
        ...
        while True:
            ...
            # print("DATA STORE")
            # print("===============================================")
            # print(str(self.data_store.subscribers))
            # print("===============================================")
            # print("+++++++++++++++ END +++++++++++++++++++++++++++")
            # print()
            # print()
...
if len(sys.argv) == 2:
    ...
    db = open('pk','wb')
    pickle.dump([set([int(sys.argv[1])]),set()],db)
    db.close()
